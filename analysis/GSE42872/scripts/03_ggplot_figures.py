...
def order_rows(mat):
    """层次聚类排序；行数 <2 时 pdist 会返回空矩阵，直接按原序返回。"""
    import numpy as _np
    if mat.shape[0] < 2:
        return _np.arange(mat.shape[0])
    return leaves_list(linkage(pdist(mat), "average"))
...
order = order_rows(expr.T.values)
...
p4 = (ggplot(v, aes("logFC", "nlp", color="direction"))
      ...
      + theme_pub())
if len(lab):
    p4 = p4 + geom_text(lab, aes(label="symbol"), size=7, color="#1A1A1A",
...
# --------------------------------------------------------- Fig 6 top50 热图
print("== Fig6 top50 热图 ==")
sig = de[de.direction != "n.s."]
has_symbol = sig.symbol.notna().any()
if has_symbol:
    top = sig[sig.symbol.notna()].drop_duplicates("symbol").head(50)
    row_labels = top.symbol.values
else:
    # 没有注释时用探针 ID，保证图照样出得来
    print("  (无 symbol 注释，热图改用探针 ID 标行)")
    top = sig.drop_duplicates("probe").head(50)
    row_labels = top.probe.astype(str).values

if len(top) < 2:
    print(f"  跳过：可用于热图的显著基因只有 {len(top)} 个")
else:
  sub = expr.loc[top.probe.astype(str)]
  z = sub.sub(sub.mean(axis=1), axis=0).div(sub.std(axis=1).replace(0, 1), axis=0)
  z.index = row_labels
  go = order_rows(z.values)
  so = order_rows(z.T.values)
  ...
  ttl = ("Top 50 differentially expressed genes" if has_symbol
         else "Top 50 differentially expressed probes")
  p6 = (ggplot(h, aes("label", "gene", fill="z"))
        ...
        + labs(title=ttl, subtitle="row-wise z-score", x="", y="")
        + theme_pub(8)
...
if len(sel) == 0:
    print("  跳过：没有匹配到 MAPK 通路基因（通常是缺少探针注释）")
rows = []
for g in mapk:
